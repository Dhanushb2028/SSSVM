...
import { assignHostelRoomAction } from "@/server/actions/hostel-actions";
...
export function HostelSection({
  ...
  hostelRooms: { id: string; roomNumber: string; hostel: { name: string } }[];
}) {
  const [state, formAction, pending] = useActionState<FormState, FormData>(assignHostelRoomAction, {});
  const [selected, setSelected] = React.useState(hostelRoomId ?? NONE);
  ...
  return (
    <Card>
      ...
      <CardContent>
        <form action={formAction} className="flex flex-wrap items-end gap-2">
          <input type="hidden" name="studentId" value={studentId} />
          <input type="hidden" name="hostelRoomId" value={selected === NONE ? "" : selected} />
          <Select value={selected} onValueChange={setSelected}>
            <FormField id="hostel-room" label="Room">
              <SelectTrigger className="w-64">
